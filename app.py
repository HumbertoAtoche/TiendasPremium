...
def generar_pdf_boleta(datos_b):
    ...
    story.append(Paragraph(f"<b>{str(datos_b['empresa'])}</b>", title_style))
    story.append(Paragraph(f"RUC: {str(datos_b['ruc'])} | PERÍODO DE PAGO: {str(datos_b['periodo'])}", ParagraphStyle('Sub', alignment=1, fontSize=10, fontName='Helvetica-Bold')))
...
# --- MÓDULO NUEVO: BOLETAS DE PAGO & FERIADOS (SOLO ADMIN) ---
    ...
    with tab_boleta:
        ...
        if not st.session_state.asistencia.empty:
            df_asist_b = st.session_state.asistencia[
                (st.session_state.asistencia["nombre"] == colab_b_sel)
            ].copy()
            ...
            if not df_asist_b.empty:
                ...
                if not df_asist_b.empty:
                    ...
                    for fecha_d, grupo_dia in df_asist_b.groupby("fecha"):
                        dias_asistidos_auto += 1
                        _, mins_e, _, _ = calcular_jornada_y_horas_extras(grupo_dia)
                        mins_extras_auto += mins_e
                        
                        if str(fecha_d) in lista_feriados_str:
                            feriados_trabajados_auto += 1

        # 2. Descuadres en el Mes (solo faltantes)
        descuadre_caja_auto = 0.0
        if not st.session_state.descuadres.empty:
            df_desc_b = st.session_state.descuadres[
                (st.session_state.descuadres["nombre"] == colab_b_sel)
            ].copy()
            ...
            if not df_desc_b.empty:
                ...
                if not df_desc_b.empty:
                    faltantes_df = df_desc_b[df_desc_b["monto"].astype(float) < 0]
                    descuadre_caja_auto = abs(faltantes_df["monto"].astype(float).sum())

        # 3. Adelantos de Sueldo Aprobados en el Mes
        adelanto_sueldo_auto = 0.0
        if not st.session_state.solicitudes.empty:
            ...
            if not df_sol_b.empty:
                ...
                if not df_sol_b.empty:
                    adelanto_sueldo_auto = df_sol_b["monto_adelanto"].astype(float).sum()

        st.markdown("---")
        st.markdown("##### ⚙️ Valores de Cálculo (Ajustables)")

        c_calc1, c_calc2, c_calc3, c_calc4 = st.columns(4)
        
        sueldo_basico_in = c_calc1.number_input("Sueldo Básico (S/.)", min_value=0.0, value=530.00, step=10.0, format="%.2f")
        dias_trabajados_in = c_calc2.number_input("Días Trabajados", min_value=0, max_value=31, value=int(dias_asistidos_auto), step=1)
        
        dias_faltas_calc = max(0, 30 - dias_trabajados_in) if dias_trabajados_in <= 30 else 0
        dias_faltas_in = c_calc3.number_input("Días de Faltas", min_value=0, max_value=31, value=int(dias_faltas_calc), step=1)
        
        feriados_trab_in = c_calc4.number_input("Feriados Trabajados", min_value=0, max_value=10, value=int(feriados_trabajados_auto), step=1)

        c_calc5, c_calc6, c_calc7 = st.columns(3)
        
        hrs_extras_val = round(mins_extras_auto / 60.0, 2)
        hrs_extras_in = c_calc5.number_input("Horas Extras (Hrs)", min_value=0.0, value=float(hrs_extras_val), step=0.5, format="%.2f")
        adelantos_in = c_calc6.number_input("Adelantos de Sueldo (S/.)", min_value=0.0, value=float(adelanto_sueldo_auto), step=5.0, format="%.2f")
        descuadre_in = c_calc7.number_input("Descuadre / Faltante Caja (S/.)", min_value=0.0, value=float(descuadre_caja_auto), step=1.0, format="%.2f")

        # --- FÓRMULAS DE CÁLCULO ---
        costo_dia = sueldo_basico_in / 30.0 if sueldo_basico_in > 0 else 0.0
        costo_hora = costo_dia / 5.75 if costo_dia > 0 else 0.0  # Jornada diaria de 5.75 hrs (5h 45m)
        
        monto_feriados = feriados_trab_in * (costo_dia * 2)
        monto_horas_extras = hrs_extras_in * (costo_hora * 1.25)
        monto_faltas = dias_faltas_in * costo_dia

        total_ingresos = sueldo_basico_in + monto_feriados + monto_horas_extras
        total_descuentos = adelantos_in + monto_faltas + descuadre_in
        ...
        datos_boleta_generada = {
            ...
            "dias_trabajados": dias_trabajados_in,
            ...
            "adelanto_sueldo": adelantos_in,
            "descuadre_caja": descuadre_in,
            "total_ingresos": total_ingresos,
        ...
        st.markdown("---")
        st.markdown("##### 👁️ Previsualización de Boleta de Pago")

        html_boleta = f"""
        <div class="boleta-container">
            <div class="boleta-header-title">
                {datos_boleta_generada['empresa']}<br>
                <span style="font-size: 0.85rem; font-weight: 500; color: #4B5563;">RUC: {datos_boleta_generada['ruc']} | BOLETA DE PAGO - {datos_boleta_generada['periodo']}</span>
            </div>
            
            <table class="boleta-table">
                <tr>
                    <td style="width:15%; font-weight:700; background-color:#f9fafb;">COLABORADOR:</td>
                    <td style="width:35%; font-weight:600;">{datos_boleta_generada['colaborador']}</td>
                    <td style="width:15%; font-weight:700; background-color:#f9fafb;">DNI:</td>
                    <td style="width:35%;">{datos_boleta_generada['dni']}</td>
                </tr>
                <tr>
                    <td style="font-weight:700; background-color:#f9fafb;">CARGO:</td>
                    <td>{datos_boleta_generada['cargo']}</td>
                    <td style="font-weight:700; background-color:#f9fafb;">FECHA INGRESO:</td>
                    <td>{datos_boleta_generada['fecha_inicio']}</td>
                </tr>
                <tr>
                    <td style="font-weight:700; background-color:#f9fafb;">DÍAS LABORADOS:</td>
                    <td>{datos_boleta_generada['dias_trabajados']} días</td>
                    <td style="font-weight:700; background-color:#f9fafb;">DÍAS FALTAS:</td>
                    <td>{datos_boleta_generada['dias_faltas']} días</td>
                </tr>
                <tr>
                    <td style="font-weight:700; background-color:#f9fafb;">FERIADOS TRAB.:</td>
                    <td>{datos_boleta_generada['feriados_trabajados']} días</td>
                    <td style="font-weight:700; background-color:#f9fafb;">HORAS EXTRAS:</td>
            ...
            <table class="boleta-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="text-align:right;">{datos_boleta_generada['sueldo_basico']:.2f}</td>
                        <td style="text-align:right;">0.00</td>
                    </tr>
                    <tr>
                        <td>PAGO FERIADOS TRABAJADOS (DOBLE)</td>
                        <td style="text-align:center;">{datos_boleta_generada['feriados_trabajados']} días</td>
                        <td style="text-align:right;">{datos_boleta_generada['monto_feriados']:.2f}</td>
                        <td style="text-align:right;">0.00</td>
                    </tr>
                    <tr>
                        <td>HORAS EXTRAS TRABAJADAS (125%)</td>
                        <td style="text-align:center;">{datos_boleta_generada['horas_extras_hrs']:.2f} hrs</td>
                        <td style="text-align:right;">{datos_boleta_generada['monto_horas_extras']:.2f}</td>
                        <td style="text-align:right;">0.00</td>
                    </tr>
                    <tr>
                        ...
                        <td style="text-align:right;">0.00</td>
                        <td style="text-align:right;">{datos_boleta_generada['adelanto_sueldo']:.2f}</td>
                    </tr>
                    <tr>
                        <td>DESCUENTOS POR FALTAS</td>
                        <td style="text-align:center;">{datos_boleta_generada['dias_faltas']} días</td>
                        <td style="text-align:right;">0.00</td>
                        <td style="text-align:right;">{datos_boleta_generada['monto_faltas']:.2f}</td>
                    </tr>
                    <tr>
                        <td>DESCUADRE / FALTANTE DE CAJA</td>
                        <td style="text-align:center;">-</td>
                        <td style="text-align:right;">0.00</td>
                        <td style="text-align:right;">{datos_boleta_generada['descuadre_caja']:.2f}</td>
                    </tr>
                    <tr style="font-weight:700; background-color:#f9fafb;">
                        <td>TOTALES</td>
                        <td></td>
                        <td style="text-align:right;">S/. {datos_boleta_generada['total_ingresos']:.2f}</td>
                        <td style="text-align:right;">S/. {datos_boleta_generada['total_descuentos']:.2f}</td>
                    </tr>
                    <tr style="font-weight:800; font-size:0.95rem; background-color:#f0fdf4; color:#15803d;">
                        <td colspan="2">NETO A PAGAR</td>
                        <td colspan="2" style="text-align:right; font-size:1.1rem;">S/. {datos_boleta_generada['neto_pagar']:.2f}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        excel_bytes = generar_excel_boleta(datos_boleta_generada)
        col_d1.download_button(
            label="📊 Descargar Boleta en Excel",
            data=excel_bytes,
            file_name=f"Boleta_{colab_b_sel}_{datos_boleta_generada['periodo']}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )

        if REPORTLAB_AVAILABLE:
            pdf_bytes = generar_pdf_boleta(datos_boleta_generada)
            if pdf_bytes:
                col_d2.download_button(
                    label="📄 Descargar Boleta en PDF",
                    data=pdf_bytes,
                    file_name=f"Boleta_{colab_b_sel}_{datos_boleta_generada['periodo']}.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
        else:
            col_d2.info("Instala 'reportlab' para habilitar la descarga en PDF.")

elif choice == "Solicitudes y Permisos":
    st.markdown("""
        <div class="market-header">
            <h1>Gestión de Solicitudes y Permisos</h1>
            <p>Revisión, aprobación y rechazo de solicitudes administrativas</p>
        </div>
    """, unsafe_allow_html=True)

    if not st.session_state.solicitudes.empty:
        df_sols = st.session_state.solicitudes.copy()
        
        tab_p, tab_h = st.tabs(["⏳ Pendientes de Aprobación", "🗂️ Historial General"])

        with tab_p:
            df_pend = df_sols[df_sols["estado"] == "Pendiente"]
            if not df_pend.empty:
                for idx_s, r_s in df_pend.iterrows():
                    with st.expander(f"📌 {r_s['tipo_solicitud']} — {r_s['nombre']} ({r_s['fecha_registro']})", expanded=True):
                        st.markdown(f"**Trabajador:** {r_s['nombre']} (DNI: {r_s['dni']})")
                        st.markdown(f"**Tipo:** {r_s['tipo_solicitud']}")
                        
                        if r_s['tipo_solicitud'] == "Permiso Laboral":
                            st.markdown(f"**Fecha Solicitada:** {r_s['fecha_permiso']}")
                        else:
                            st.markdown(f"**Monto Adelanto:** S/. {float(r_s['monto_adelanto']):.2f}")

                        st.markdown(f"**Motivo:** {r_s['motivo']}")

                        resp_admin = st.text_input(f"Respuesta / Observación del Admin", key=f"resp_{r_s['id_solicitud']}")
                        
                        c_ap1, c_ap2 = st.columns(2)
                        if c_ap1.button("✅ Aprobar Solicitud", key=f"btn_ap_{r_s['id_solicitud']}", use_container_width=True):
                            st.session_state.solicitudes.loc[st.session_state.solicitudes["id_solicitud"] == r_s["id_solicitud"], "estado"] = "Aprobado"
                            st.session_state.solicitudes.loc[st.session_state.solicitudes["id_solicitud"] == r_s["id_solicitud"], "respuesta_admin"] = resp_admin
                            actualizar_hoja_completa("Solicitudes", st.session_state.solicitudes)
                            st.success(f"Solicitud {r_s['id_solicitud']} aprobada")
                            time.sleep(0.3)
                            st.rerun()

                        if c_ap2.button("❌ Rechazar Solicitud", key=f"btn_rec_{r_s['id_solicitud']}", use_container_width=True):
                            st.session_state.solicitudes.loc[st.session_state.solicitudes["id_solicitud"] == r_s["id_solicitud"], "estado"] = "Rechazado"
                            st.session_state.solicitudes.loc[st.session_state.solicitudes["id_solicitud"] == r_s["id_solicitud"], "respuesta_admin"] = resp_admin
                            actualizar_hoja_completa("Solicitudes", st.session_state.solicitudes)
                            st.error(f"Solicitud {r_s['id_solicitud']} rechazada")
                            time.sleep(0.3)
                            st.rerun()
            else:
    ...
    st.markdown("""
        <div class="market-header">
            ...
            <p>Auditoría global de sobrantes y faltantes de caja</p>
        </div>
    """, unsafe_allow_html=True)

    if not st.session_state.descuadres.empty:
        df_desc_tot = st.session_state.descuadres.copy()
        
        col_df1, col_df2 = st.columns(2)
        colabs_all = ["Todos"] + df_desc_tot["nombre"].unique().tolist()
        sel_c_desc = col_df1.selectbox("Filtrar por Colaborador", colabs_all)
        
        if sel_c_desc != "Todos":
            df_desc_tot = df_desc_tot[df_desc_tot["nombre"] == sel_c_desc]
        ...
        excel_desc = to_excel(df_desc_tot)
        st.download_button("📊 Exportar Historial a Excel", excel_desc, "Historial_Descuadres.xlsx", "application/vnd.ms-excel")
    else:
        st.info("No se registran descuadres de caja en el sistema.")

elif choice == "Historial de Asistencias":
    st.markdown("""
        <div class="market-header">
            <h1>Historial Global de Asistencia</h1>
            <p>Registro completo de marcaciones, ingresos, salidas y coberturas extras</p>
        </div>
    """, unsafe_allow_html=True)

    if not st.session_state.asistencia.empty:
        df_asist_tot = st.session_state.asistencia.copy()
        
        col_a1, col_a2 = st.columns(2)
        colabs_a_list = ["Todos"] + df_asist_tot["nombre"].unique().tolist()
        sel_a_col = col_a1.selectbox("Filtrar por Colaborador", colabs_a_list)

        if sel_a_col != "Todos":
            df_asist_tot = df_asist_tot[df_asist_tot["nombre"] == sel_a_col]
        ...
        excel_asist = to_excel(df_asist_tot)
        st.download_button("📊 Exportar Asistencias a Excel", excel_asist, "Historial_Asistencia.xlsx", "application/vnd.ms-excel")
    else:
        st.info("No se registran marcaciones de asistencia en el sistema.")

# --- FOOTER GLOBAL ---
st.markdown("""
    <div class="app-footer">
        <strong>Tiendas Premium E.I.R.L.</strong> — Sistema de Gestión Interna & Control de Asistencia<br>
        Desarrollado con Streamlit & Google Sheets API • Versión 2.5
    </div>
""", unsafe_allow_html=True)
